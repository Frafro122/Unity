...
public class RandomCubesGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        List<int> pozycje_x = new List<int>(Enumerable.Range(0, Convert.ToInt32(rend.bounds.size.x)).OrderBy(x => Guid.NewGuid()).Take(howmuchblocks));
        List<int> pozycje_z = new List<int>(Enumerable.Range(0, Convert.ToInt32(rend.bounds.size.z)).OrderBy(x => Guid.NewGuid()).Take(howmuchblocks));

        for (int i = 0; i < howmuchblocks; i++)
        {
